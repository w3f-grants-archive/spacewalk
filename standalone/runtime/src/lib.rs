...
use frame_support::{
	...
	traits::{
		Contains, Currency as PalletCurrency, EnsureOneOf, ExistenceRequirement, Imbalance,
		OnUnbalanced,
	},
	PalletId,
};
...
pub use primitives::{
	self, AccountId, Balance, BlockNumber, CurrencyId, CurrencyId::Token, CurrencyInfo, Hash,
	Moment, Nonce, Signature, SignedFixedPoint, SignedInner, TokenSymbol, UnsignedFixedPoint,
	UnsignedInner, DOT, IBTC, INTR, KBTC, KINT, KSM,
};
...
pub const BITCOIN_BLOCK_SPACING: BlockNumber =
	BITCOIN_SPACING_MS / MILLISECS_PER_BLOCK as BlockNumber;
...
impl Contains<Call> for BaseCallFilter {
	fn contains(call: &Call) -> bool {
		if matches!(
			call,
			Call::System(_) | Call::Timestamp(_) | Call::Sudo(_) | Call::TechnicalCommittee(_)
		) {
...
impl annuity::BlockRewardProvider<AccountId> for EscrowBlockRewardProvider {
	...
	fn deposit_stake(from: &AccountId, amount: Balance) -> DispatchResult {
		<EscrowRewards as reward::Rewards<AccountId, Balance, CurrencyId>>::deposit_stake(
			from, amount,
		)
	}
