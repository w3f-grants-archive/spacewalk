...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		Deposit(CurrencyIdOf<T>, <T as frame_system::Config>::AccountId, BalanceOf<T>),
		/// User initiated a redeem.
		Redeem {
			asset_code: Vec<u8>,
			asset_issuer: Vec<u8>,
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn redeem(
			...
			let stellar_user_address = match T::AddressConversion::lookup(pendulum_account_id.clone()) {
				Ok(address) => address,
				Err(_) => return Err(sp_runtime::DispatchErrorWithPostInfo {
					post_info: Default::default(),
					error: DispatchError::Other("Address conversion unsuccessful"),
				}),
			};
